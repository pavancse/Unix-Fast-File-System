...
class TimerInterruptHandler implements VoidFunctionPtr {
	...
	public void call(Object pDummy) {
		...
		if (Interrupt.getStatus() != Interrupt.IdleMode) {
			// Yield on return
			//System.out.println("TimerInterruptHandler Call "+Thread.currentThread().getName());
...
public abstract class JNachos {
	...
	public static void initialize(String args[]) {
		...
		mFileSystem = new NachosFileSystem(format);//Before it was NachosFileSystem
	 ...
	 **/
	public static void startProcess(String[] filename) {//add []
		for (int i=1;i<filename.length;i++) {

			OpenFile executable = mFileSystem.open(filename[i]);

			if (executable == null) {
				Debug.print('t', "Unable to open file" + filename[i]);
				return;
			}

			NachosProcess p = new NachosProcess("process"+i);



			AddrSpace space = new AddrSpace(executable);
			p.setSpace(space);
			p.setProcessSwapFile(space.getSwapFile());

			p.fork(new VoidFunctionPtr() {
				@Override
		...
		OpenFile executable = mFileSystem.open(filename[0]);

		// If the file does not exist
		if (executable == null) {
			Debug.print('t', "Unable to open file" + filename[0]);
			return;
		}
		...
		getCurrentProcess().setProcessSwapFile(space.getSwapFile());
