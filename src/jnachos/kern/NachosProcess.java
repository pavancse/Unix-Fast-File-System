...
public class NachosProcess implements Runnable {
	...
	/**
	 * Indicated the process ID of process
	 */
	private int mProcessID;

	/**
	 * To track the process ID
	 */
	private static int previousProcessID = 0;

	/**
	 * Map to track when a process is joined and resume back once joined process finish
	 */
	private static HashMap<NachosProcess, Integer> waitOnJoinProcess = new HashMap<>();

	/**
	 * Keep track of all the process created
	 */
	private static LinkedList<NachosProcess> processList = new LinkedList<NachosProcess>();

	/**
	 * Flag to determine whether the process is forked by some some process or not
	 */
	private boolean forkedChild;


	private OpenFile processSwapFile;
	...
	public synchronized void resume() {
		...
		AddrSpace space = getSpace();
		if (mStarted) {
			this.restoreUserState();
			this.notify();
		} else {
			// If this thread has not yet run, start it
			//First time init the registers

			if(space != null){
				space.initRegisters();
			}
			if(this.isForkedChild()){
				this.restoreUserState();
			}
			mThread.start();
		...
		if(space != null) {
			space.restoreState();//restoreState before start
		}
	}
	...
	public void finish() {
		...
		//check whether any process is waiting for calling process to finish
		// if yes, resume the waiting process before calling process finish
		NachosProcess waitingProcess = JNachos.getCurrentProcess().checkJoinedWaitingThread();

		//Invoke to clean the memory before the process terminates
		if(null != this.getSpace())
		AddrSpace.cleanAddrSpcae(this.getSpace());


		if(waitingProcess != null){
			waitingProcess.writeUserRegister(2,Machine.readRegister(4));
...
//		System.out.println("Switching from process "+ oldProcess.getmProcessID() + " to process " + pNextProcess.getmProcessID());
	...
	/**
	 *
	 * Create the process id for every new process when it is created
	 * @return The process id if created process.
	 */
	private int createProcessID(){
		previousProcessID = previousProcessID + 1;
		return previousProcessID;
	}

	/**
	 * The the process id of the invoking process
	...
	/**
	 * Write the a value to user register
	 * @param num user register index
	 * @param value the value need to be added for that index
	 */
	public void writeUserRegister(int num, int value){
		assert ((num >= 0) && (num < Machine.NumTotalRegs));
		Debug.print('m', "WriteUserRegister " + num + ", value" + value);
		this.mUserRegisters[num] = value;
	}

	/**
	 * check whether the created process exists
	 * @param pid the process if of the pid that need to verify its state
	 * @return true if process exists else false
	 */
	public boolean checkProcessExists(int pid){
		for(NachosProcess process : processList){
			if (process.getmProcessID() == pid)
				return true;
		}
		return false;
	}
	...
	/**
	 * store the calling thread and called process details
	 * used to track the when join is invoked on a process
	 * @param pid process id of calling process
	 */
	public void addJoinThread(int pid){
		waitOnJoinProcess.put(this, pid);
	}

	/**
	 * verify any process is waiting for its execution to finish
	 * @return if any process waiting return its process details
	 */
	public NachosProcess checkJoinedWaitingThread(){
		Iterator it = waitOnJoinProcess.entrySet().iterator();
		while (it.hasNext()) {
			Map.Entry pair = (Map.Entry)it.next();
			if(this.getmProcessID() == (Integer) pair.getValue()){
				it.remove();
				return (NachosProcess) pair.getKey();
			}
		}
		return null;
	}
	...
	/**
	 * verify whether the invoking process is a forked process or not
	 * @return
	 */
	public boolean isForkedChild() {
		return forkedChild;
	}

	/**
	 * set when the child is forked by other process
	 * @param forkedChild if forked chile true else false
	 */
	public void setForkedChild(boolean forkedChild) {
		this.forkedChild = forkedChild;
	}


	public OpenFile getProcessSwapFile(){
		return processSwapFile;
	}

	public void setProcessSwapFile(OpenFile swapFile){
		processSwapFile = swapFile;
	}

	public static NachosProcess getProcessOnID(int processID){
		for(NachosProcess process : processList){
			if (process.getmProcessID() == processID)
				return process;
		}
		return null;
	}
